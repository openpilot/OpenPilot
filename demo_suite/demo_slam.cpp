...
void demo_slam01_main(world_ptr_t *world) {
  ...
  boost::shared_ptr< DataManagerActiveSearch<RawImage,SensorPinHole> >
    dmPt11( new DataManagerActiveSearch<RawImage,SensorPinHole>() );
  ...
  for (int t = 1; t <= NFRAME;)
      ...
      if (had_data)
	{
	  ...
	  cout << "dt: " << (int)(1000*robPtr1->dt_or_dx) << "ms. "
	       << "p: " << 100*p << " cm."
	       << "e: " << (180.0 / 3.14)*e << " deg." << endl;
	}
      worldPtr->display_mutex.unlock();
      mutex_chrono.reset();
  ...
  std::cout << "average time : " << total_chrono.elapsed()/NFRAME << " ms,"
	    << "max frame time " << max_dt << std::endl;
  std::cout << "\nFINISHED !" << std::endl;
