/**
  ...
  *                 
  * @note
  *  Memory-to-Memory transfer is possible by setting the address of the memory into the Peripheral
  *  registers. In this mode, Circular mode and Double Buffer mode are not allowed.
  *  
  * @note
  *  The FIFO is used mainly to reduce bus usage and to allow data packing/unpacking: it is possible
  *  to set different Data Sizes for the Peripheral and the Memory (ie. you can set Half-Word data size
  *  for the peripheral to access its data register and set Word data size for the Memory to gain in
  *  access time. Each two Half-words will be packed and written in a single access to a Word in the Memory).
  *    
  * @note  
  *  When FIFO is disabled, it is not allowed to configure different Data Sizes for Source and Destination.
  *  In this case the Peripheral Data Size will be applied to both Source and Destination.               
  *
  *      
  *  @endverbatim
  *                                  
