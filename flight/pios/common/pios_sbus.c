                       ...
                       bool non_inverted)
{
    ...
    *sbus_id = (uint32_t)sbus_dev;
    ...
    if (non_inverted) {
        GPIO_WriteBit(cfg->inv.gpio, cfg->inv.init.GPIO_Pin, cfg->gpio_inv_disable); // look for non inverted sbus signal (Not Standard S.Bus.)
    } else {
        GPIO_WriteBit(cfg->inv.gpio, cfg->inv.init.GPIO_Pin, cfg->gpio_inv_enable); // look for inverted sbus signal (Standard S.Bus.)
    }
