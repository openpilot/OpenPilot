...
int32_t PIOS_Flash_Norsim_ReadData(uintptr_t flash_id, uint32_t addr_val, uint8_t *data, uint16_t len)
{
	...
	u32 *buf = (u32 *)data;
	u32 *read_addr = (u32 *) ( (uint8_t *)(sim->word) + addr_val );
	while (nwords > 0) {
			*buf = *read_addr;
			buf++;
			read_addr++;
...
int32_t PIOS_Flash_Norsim_WriteData(uintptr_t flash_id, uint32_t addr_val, uint8_t *data, uint16_t len)
{
	...
	uint16_t nwords = len / 4;
	u32 *buf = (u32 *)data;
	uint32_t *write_addr = (uint32_t*) ((uint8_t *)(sim->word) + addr_val);
	while (nwords > 0) {
		ynorsim_wr_one_word32(sim, write_addr, *buf);
		write_addr++;
		buf++;
		nwords--;
	}
...
int32_t PIOS_Flash_Norsim_EraseSector(uintptr_t flash_id, uint32_t addr)
{
	...
	uint8_t * write_addr = (uint8_t *)(sim->word) + addr;
	memset(write_addr, 0xFF, sim->block_size_bytes);
