...
void PIOS_Board_Init(void)
{
  ...
  /*  if (PIOS_SPI_Init(&pios_spi_telem_id, &pios_spi_telem_cfg)) {
	...
	/*if (PIOS_FLASHFS_Logfs_Init(&pios_uavo_settings_fs_id, &flashfs_external_cfg, &pios_jedec_flash_driver, flash_id)) {
        PIOS_DEBUG_Assert(0);
    }*/
...
//PIOS_DELAY_WaitmS(500);
   ...
   // PIOS_WDG_Init();
    ...
    case HWSETTINGS_RM_MAINPORT_GPS:
        PIOS_Board_configure_com(&pios_main_usart_cfg, PIOS_COM_GPS_RX_BUF_LEN, -1, &pios_usart_com_driver, &pios_com_gps_id);
        break;
...
#if defined(PIOS_INCLUDE_DEBUG_CONSOLE)
        {
            PIOS_Board_configure_com(&pios_main_usart_cfg, 0, PIOS_COM_DEBUGCONSOLE_TX_BUF_LEN, &pios_usart_com_driver, &pios_com_debug_id);
        }
#endif /* PIOS_INCLUDE_DEBUG_CONSOLE */
        break;
    ...
    case HWSETTINGS_RM_FLEXIPORT_I2C:
        ...
        {
            if (PIOS_I2C_Init(&pios_i2c_flexiport_adapter_id, &pios_i2c_flexi_cfg)) {
                PIOS_Assert(0);
            }
        }
