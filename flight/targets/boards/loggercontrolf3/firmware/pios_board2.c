...
static const struct pios_mpu6050_cfg pios_mpu6050_cfg = {
	...
	.gyro_range = PIOS_MPU6050_SCALE_2000_DEG,
...
void PIOS_Board_Init(void) {
	...
	//PIOS_FLASHFS_Format();
	//PIOS_Flash_Jedec_Init(pios_spi_flash_id, 1, &flash_m25p_cfg);
	//PIOS_FLASHFS_Init(&flashfs_m25p_cfg);
	//PIOS_Flash_Jedec_Init(pios_spi_flash_id, 0, &flash_w25x_cfg);//slave on 0!!!!!
	//PIOS_FLASHFS_Init(&flashfs_w25x_cfg);
	uintptr_t flash_id;
	if (PIOS_Flash_Jedec_Init(&flash_id, pios_spi_flash_id, 0)) {
		PIOS_DEBUG_Assert(0);
	}
	if (PIOS_FLASHFS_Logfs_Init(&pios_user_fs_id, &flashfs_w25x_cfg, &pios_jedec_flash_driver, flash_id)) {
		PIOS_DEBUG_Assert(0);
	}
#elif defined (PIOS_INCLUDE_FLASH_INTERNAL)
	...
	if(Radio_present==1)
	{
		usb_hid_present = false;//true; //to modify
	}
	else
	{
		usb_hid_present = true;
	...
	//hwsettings_mainport=HWSETTINGS_RM_MAINPORT_TELEMETRY;
	//HwSettingsRM_MainPortSet(hwsettings_mainport);
	switch (hwsettings_mainport) {
		case HWSETTINGS_RM_MAINPORT_DISABLED:
			break;
		case HWSETTINGS_RM_MAINPORT_TELEMETRY:
			{
#if defined(PIOS_INCLUDE_LOG)
			uint32_t pios_usart_id;
			if (PIOS_USART_Init(&pios_usart_id, &pios_usart_main_cfg)) {
				PIOS_Assert(0);
			}
			...
			if (PIOS_COM_Init(&pios_com_telem_rf_id, &pios_usart_com_driver, pios_usart_id,
					NULL, 0,
		...
		case HWSETTINGS_RM_MAINPORT_DEBUGCONSOLE:
#if defined(PIOS_INCLUDE_DEBUG_CONSOLE)
			{
				...
				//PIOS_COM_SendFormattedStringNonBlocking(pios_com_debug_id, "\r\n");
			}
#endif	/* PIOS_INCLUDE_DEBUG_CONSOLE */
			break;
...
#if defined(PIOS_INCLUDE_MPU6050)
	hwsettings_flexiport=HWSETTINGS_RM_FLEXIPORT_I2C;
	HwSettingsRM_FlexiPortSet(&hwsettings_flexiport);
#endif
...
#if defined(PIOS_INCLUDE_MPU6050)
	PIOS_MPU6050_Init(pios_i2c_flexiport_adapter_id, &pios_mpu6050_cfg);
#endif
}
