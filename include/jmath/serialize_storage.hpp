...
namespace boost { namespace numeric { namespace ublas {
    ...
    private:
        ...
        { 
          ...
          ar & boost::serialization::make_nvp("data", boost::serialization::make_binary_object(data_, size_*sizeof(value_type)));
        }
        template<class Archive>
        void load(Archive & ar, const unsigned int version)
        {
          size_type new_size;
          ar & BOOST_SERIALIZATION_NVP(new_size);
          resize_internal(new_size, value_type(), false);
          ar & boost::serialization::make_nvp("data", boost::serialization::make_binary_object(data_, size_*sizeof(value_type)));
        }
    ...
    private:
        ...
        void load(Archive & ar, const unsigned int version)
        {
          size_type new_size;
          ar & BOOST_SERIALIZATION_NVP(new_size);
          if (new_size > N) bad_size("too large size in bounded_array::load()\n").raise();
          resize(new_size);
          ar & boost::serialization::make_nvp("data", boost::serialization::make_binary_object(data_, size_*sizeof(value_type)));
        }
        BOOST_SERIALIZATION_SPLIT_MEMBER()
