...
namespace jafar {
	namespace rtslam {
		...
		vec2 SensorPinHole::projectPoint(const vec4 & k, const vec & d, const vec3 & v){
			return pixellizePoint(k, distortPoint(d, projectPointToNormalizedPlane(v)));
		}

		void SensorPinHole::projectPoint(const vec4 & k, const vec & d, const vec3 & v, vec2 & u, mat23 & U_v){
			vec2 up, ud;
			mat23 UP_v;
			mat22 UD_up, U_ud;
		...
		vec2 SensorPinHole::projectPoint(const vec3 & v){
			return projectPoint(intrinsic, distortion, v);
		}
		...
		void SensorPinHole::projectPointToNormalizedPlane(const jblas::vec3 & v, vec2 & up, mat23 & UP_v) {
		...
		jblas::vec2 SensorPinHole::distortPoint(const jblas::vec & d, const jblas::vec2 & up) {
			size_t n = d.size();
			if (n == 0)
				return up;
			else {
		...
		void SensorPinHole::distortPoint(const jblas::vec & d, const jblas::vec2 & up, vec2 & ud, mat22 & UD_up) {
			size_t n = d.size();
			jblas::vec2 R2_up;
			jblas::vec2 S_up;
		...
		jblas::vec2 SensorPinHole::pixellizePoint(const jblas::vec4 & k, const jblas::vec2 & ud) {
			double u_0 = k(0);
			double v_0 = k(1);
			double a_u = k(2);
			double a_v = k(3);
		...
		void SensorPinHole::pixellizePoint(const jblas::vec4 & k, const jblas::vec2 & ud, jblas::vec2 & u, jblas::mat22 & U_ud) {
			//double u_0 = k(0);
			//double v_0 = k(1);
			double a_u = k(2);
			double a_v = k(3);
